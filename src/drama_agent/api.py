...
class SaveMemoryRequest(BaseModel):
    user_id: str = Field("guest", max_length=128)
...
def save_user_memory(req: SaveMemoryRequest) -> Dict[str, Any]:
...
def list_user_memory(
    user_id: str = Query(default="guest", max_length=128),
    limit: int = Query(default=50, ge=1, le=200),
    full: bool = Query(default=True, description="是否返回完整问答内容"),
) -> Dict[str, Any]:
    store = get_user_memory_store()
    memories = store.list_memories(user_id, limit=limit, full=full)
    return {
...
def export_user_memories(
    ...
    return {
        ...
        "total": store.count(user_id),
        "memories": store.export_all(user_id),
    }


@app.post("/v1/memory/import")
def import_user_memories(req: ImportMemoryRequest) -> Dict[str, Any]:
    if not settings.enable_user_memory:
        raise HTTPException(status_code=400, detail="个人记忆库功能已关闭")
    try:
        store = get_user_memory_store()
        result = store.import_entries(
            req.user_id, req.memories, skip_duplicates=req.skip_duplicates,
        )
        return {"status": "ok", "user_id": req.user_id, **result}
    except Exception as e:
...
def update_user_memory(memory_id: str, req: UpdateMemoryRequest) -> Dict[str, Any]:
    if not settings.enable_user_memory:
        raise HTTPException(status_code=400, detail="个人记忆库功能已关闭")
    ...
    try:
        ...
        if not ok:
            raise HTTPException(status_code=404, detail=f"memory {memory_id} not found")
        item = store.get(memory_id, req.user_id)
        return {"status": "ok", "memory": item}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def delete_user_memory(
    ...
    store = get_user_memory_store()
    if not store.delete(memory_id, user_id):
        raise HTTPException(status_code=404, detail=f"memory {memory_id} not found")
    return {"status": "ok", "deleted": memory_id, "total": store.count(user_id)}
