...
def _resolve_project_root() -> Path:
    ...
    # 典型结构: <root>/src/drama_agent/config.py  →  parents[2] 是项目根
    try:
        candidate = Path(__file__).resolve().parents[2]
        # 验证：如果能看到 pyproject.toml / .env.example / frontend/ 说明就是它
        for marker in ("pyproject.toml", ".env.example", "frontend"):
...
class Settings(BaseSettings):
    ...
    # ---------- Embedding（当前使用本地 jieba 切分+关键词召回；若要启用外部 Embedding，可改 provider=doubao/openai）
    ...
    langchain_project: str = "drama-multi-agent"
